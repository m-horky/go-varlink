...
func main() {
	help := "usage: certification [ client | server ]"
...
func client() {
	...
	if socket == "" {
		panic("SOCKET environment variable must point to a TCP server")
	}

	log.Println("SOCKET", socket)
	conn, err := net.Dial("tcp", socket)
	c := certification.Client{Client: govarlink.NewClient(conn)}
